...
int main()
{
  ...
  std::cout << "Position of circle is (" << figures[0]->getPos().x << "; " << figures[0]->getPos().y << ")\n";

  figures[0]->move({1, 2});
  figures[1]->move(1, 2);
  std::cout << "Position " << std::string(typeid(*figures[0]).name()).substr(1) << "after moving in (2, 3) is ("
            << figures[0]->getPos().x << "; " << figures[0]->getPos().y << ")\n\n";

  std::cout << "Position of rectangle is (" << figures[1]->getPos().x << "; " << figures[1]->getPos().y << ")\n";
  ...
  std::cout << "Position " << std::string(typeid(*figures[1]).name()).substr(1) << "after moving to offset (2, 3) is ("
            << figures[1]->getPos().x << "; " << figures[1]->getPos().y << ")\n";
  return 0;
}
